...
def sampling_group_class(ground_truth,data,ignore_class=None):
    ...
    while (True):  # 重新找到随机点中心
        ...
        if ground_truth[temp_index[0]][temp_index[1]] == 0.:  # 重新找到新的中心点
           # print(temp_index[0],temp_index[1])

            for i in next_step:  # 八个方向
                if visit[temp_index[0] + i[0]][temp_index[1] + i[1]] == 0:  # 防止bfs出边界，导致截图时超出边界
                    if temp_index[0] + i[0] > 14 and temp_index[0] + i[0] < 588 and temp_index[1] + i[1] < 1986 and \
                            temp_index[1] + i[1] > 14:
...
def sixth_point_build_dataset():
        ...
        for i in range(8):  # 64group
            for j in range(8):
               ...
               # print(statistics,"+")
                if statistics >= 60:
                    # print(w_,h_)
                    sample_mat[:, i * 25:(i + 1) * 25, j * 25:(j + 1) * 25] = data_compl[:, w_ - 12:w_ + 13,
                                                                              h_ - 12:h_ + 13]
                    ...
                    while (True):
                       ...
                       # print(statistics)
                        # print(w_2,h_2)
                        if statistics >= 60:
                            sample_mat[:, i * 25:(i + 1) * 25, j * 25:(j + 1) * 25] = data_compl[:, w_2 - 12:w_2 + 13,
                                                                                      h_2 - 12:h_2 + 13]
                            sample_gt[i * 25:(i + 1) * 25, j * 25:(j + 1) * 25] = ground_truth_compl[w_2 - 12:w_2 + 13,
                                                                                  h_2 - 12:h_2 + 13]
                            break
    ...
    # temp.to_pickle("Houston_10%/sample_houston_10.pt2", compression="xz")
        ...
        sio.savemat(f"../Houston_2018_10%/train2/sample_houston_{str(epoch)}.mat",{"train":sample_mat,"label":sample_gt},do_compression = True)
        print(epoch)
